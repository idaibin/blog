...
> 更新（2026-07-14）：本文保留的是 `rustzen-admin` 在 2025 年 7 月的架构与思考。当时的 PostgreSQL、`PgPool`、权限缓存和三层模块示例属于历史实现，不代表当前代码。项目现在采用 Axum + React 的 monorepo 结构，默认使用 SQLite。完整复盘见：[rustzen-admin 一年后的变化：从基础后台到可部署的工程模板](/zh/notes/rustzen-admin-one-year-changes)。
