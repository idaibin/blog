...
Review and delivery are separate as well. `repo-review` reads the current Worktree/index, a fixed SHA/range, or a verified review package. `repo-delivery` performs separately authorized commit, push, sync, squash, cleanup, or conflict-resolution operations. One local commit carries one intent, and commit authorization never implies push. External ChatGPT review and primary-source research share the `chatgpt-review` package, and their recommendations still have to be verified against the corresponding local basis.
