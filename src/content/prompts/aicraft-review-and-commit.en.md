...
description: "A reusable AICraft prompt for strict pre-commit review, scoped staging, and safe commit planning."
pubDate: 2026-06-29
tags: ["aicraft", "prompt", "code-review", "git"]
...
2. 接口契约是否真实对齐：
   ...
   - 页面调用和字段使用
3. 是否误改无关文件或扩大范围。
4. 是否丢失已有细节：UI 宽度、样式、交互行为、用户调过的细节、既有业务逻辑。
...
如果发现真实问题，先列出最小范围修复计划，只修复本次任务相关问题，修复后运行合适的验证命令。

如果未发现阻塞问题，明确说明“未发现阻塞问题”，并说明仍未覆盖的验证风险。

## 提交规则

如果用户要求提交：

1. 先判断是否适合 1 次提交还是多次提交。
2. 如果拆分提交，输出每个 commit 的目的、文件清单、拆分理由和建议 commit message。
3. 只暂存本次任务相关文件。
4. 提交前再次检查暂存区：
   - `git diff --cached`
   - 暂存文件列表
