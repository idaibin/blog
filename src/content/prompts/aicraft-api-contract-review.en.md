...
description: "A reusable AICraft prompt for verifying backend and frontend API contract alignment."
pubDate: 2026-06-29
tags: ["aicraft", "prompt", "api", "contract-review"]
...
- query 参数
- body 参数
- 返回结构：`code`、项目真实消息字段、`data`、分页字段、错误字段
- 错误码和错误处理逻辑
- 前端 API 封装路径和 method
- 前端 API 参数映射
- TypeScript 类型定义
...
1. 先列出最小范围修复计划。
2. 只修复接口契约相关问题。
3. 修复后运行合适的验证：typecheck、build、tests、接口调用、运行日志、页面操作验证。
4. 说明每个接口的验证方式和验证结果。

如果无法完整验证，明确说明未验证的接口、原因和剩余风险。
