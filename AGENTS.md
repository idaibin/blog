...
## 内容发布流程
- 当前 `src/content/` 仍是已上线文章内容源；先完成博客草稿和站点验证，再准备外部平台版本。
- 已批准目标是按 `docs/knowledge-projection.md` 构建 `ai-handbook` 公开知识的固定版本静态投影；在 canary、锁文件和旧 URL 验证完成前，不得把目标状态当作已实现，也不得批量迁移或删除现有集合。
